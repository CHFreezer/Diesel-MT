...
Tokenizer 从零训练，不使用 Meta NLLB-200 的 `sentencepiece.bpe.model` 或 `tokenizer.json`。项目采用锁定版本 Transformers 5.x 中统一后的 `NllbTokenizer`，复用其 Hugging Face `tokenizers` BPE + Metaspace 软件实现和 NLLB 风格语言 token 控制方式；规范产物为项目自训练的 `tokenizer.json`。

版本约定：Transformers 4.x 曾并行提供基于 SentencePiece 的 slow `NllbTokenizer` 和基于 Rust `tokenizers` 的 `NllbTokenizerFast`；Transformers 5.x 已统一为继承 `TokenizersBackend` 的 `NllbTokenizer`，它本身就是 fast 实现。项目训练时显式使用 `NllbTokenizer`，保存后、CTranslate2 转换前和部署时统一通过 `AutoTokenizer` 加载，并硬断言 `tokenizer.is_fast is True`。`NllbTokenizerFast` 只作为 4.x 历史兼容名称，不进入 5.x 主线代码。

许可证边界：Transformers 的 tokenizer 软件实现采用 Apache License 2.0；单纯使用 `NllbTokenizer` / `AutoTokenizer` 不会让项目自动受 `CC-BY-NC-4.0` 约束。该风险来自直接下载或分发 Meta NLLB-200 仓库里的 tokenizer 文件、checkpoint 等资产，例如从 `facebook/nllb-200-distilled-600M` 加载或复制文件。
...
- 词表一致性：以包含全部 added/special token 的最终 `len(tokenizer)` 为准，并与模型 embedding 和输出层一致；不能用可能只表示底层 BPE 词表的 `tokenizer.vocab_size` 代替，也不能沿用 NLLB-200 的 256k 词表假设。
