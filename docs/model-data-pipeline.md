...
TD-03 将 `configs/mvp_model_data.yaml` 与 `configs/mvp_model_data.lock.json` 锁定的 MASSIVE 1.1 多平行来源转换为统一的 UTF-8/LF 人类平行样本。已完成的 v1 只生成 9 组/18 路；2026-07-16 范围修正后 TD-03 退回 `pending`，必须使用新合同增加 `zh-CN--zh-TW` 第 10 组，并以新身份发布 addendum 或 composite，不覆盖 v1。
...
- `data/model/corpus/mvp/human_parallel.jsonl`：当前 v1 的 9 组规范样本，保留不变。新版本每个 MASSIVE `(partition,id)` 对齐组必须生成 10 个无向模型关系样本；反向扩展为 20 个训练路由留给 TD-04。
