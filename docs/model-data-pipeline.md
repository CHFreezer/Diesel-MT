...
TD-03 将 `configs/mvp_model_data.yaml` 与 `configs/mvp_model_data.lock.json` 锁定的 MASSIVE 1.1 多平行来源转换为统一的 UTF-8/LF 人类平行样本。当前 schema v2 已正式发布到标准根 `data/model/corpus/mvp/`，生成 10 组关系：164,778 条清洗后无向记录、432 条拒绝；TD-03 manifest SHA-256 为 `f90b861311f90791ca651f914340dac7c073f221ec39c0759d02b2d6aefaa636`。第二个完全离线 fresh/resume 构建逐文件字节一致；原 18 路 M0 v1 语料显式保存在 `data/model/history/m0-v1/`。
...
- `data/model/corpus/mvp/human_parallel.jsonl`：当前 10 组规范样本；每个 MASSIVE `(partition,id)` 对齐组最多生成 10 个无向模型关系样本，反向扩展为 20 个训练路由留给 TD-04。
- `data/model/history/m0-v1/corpus/mvp/human_parallel.jsonl`：冻结的 9 组/18 路历史输入，仅供 D0/D1 v1 证据链使用。
