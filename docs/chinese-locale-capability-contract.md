...
2026-07-17 再明确能力语义，中文数据分为三个类别：

- `zho_Hans`：现代标准书面中文，使用简体字；直接对齐冻结 FLORES-200 `zho_Hans`。
- `zho_Hant`：现代标准书面中文，使用繁体字；直接对齐冻结 FLORES-200 `zho_Hant`，以台湾标准繁体作为主要输出规范。香港、澳门的正式书面繁体属于同一语义类别，可以补充进入，但不得把明显地区用词强制当作唯一答案。
- 粤语/广东话：语法、词汇和语义习惯独立于现代标准书面中文；不论使用繁体还是简体书写，都属于单独语言能力。当前五标签/20 路模型不包含粤语，`yue_Hant`、粤语书面语和广东话口语绝不能映射为 `zho_Hant`。未来如需支持，必须新增独立语言标签、token、训练路线和评测合同。

因此，`zh-TW`、`zh-HK`、`zh-MO` 只描述上游来源：台湾来源是当前繁体主体和输出基线，港澳标准书面繁体可作为有效补充；来源地区本身既不能把标准书面繁体拆成多种模型语言，也不能把粤语并入繁体中文。generic `zh/cmn` 若缺少语言与脚本 provenance 仍不准入。工具简转繁只能作为显式 synthetic augmentation，不能替代原生繁体 source/human anchor。
