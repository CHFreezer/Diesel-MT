...
- **Active workflow**: `work/plan/mvp-model-training.md`, `work/todo/mvp-model-training.md`, and `work/task/mvp-model-training/`. TD-01 through TD-16B are completed. TD-16A merged the hardware-configurable high-throughput trainer. TD-16B then trained the complete old 226,218-directed-record M0 through the authorized 15k early-stop boundary, but rejected every checkpoint for later stages: the corpus has only 11,411 semantic/alignment groups and MASSIVE locale adaptation does not satisfy generic MT entity fidelity. TD-16C is now active and returns the workflow to human parallel-corpus preparation; TD-16D through TD-16F, TD-17, and TD-18 remain pending. Formal test remains unconsumed. The frozen M0/D0/D1, A/B, and old-M0 long-run evidence remain immutable. The frozen tokenizer already has both Chinese tags and must not be rebuilt. Do not reinterpret the random deployment checkpoint, M1 memorization checkpoint, TD-16 A/B candidates, or rejected old-M0 checkpoints as a trained final MVP model.
