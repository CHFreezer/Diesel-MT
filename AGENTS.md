...
- **Active workflow**: `work/plan/mvp-model-training.md`, `work/todo/mvp-model-training.md`, and `work/task/mvp-model-training/`. TD-01 through TD-15 are completed. The original TD-16 source-matched A/B diagnostic also completed two bounded runs on the 44,313-record common cohort and selected human-only step 1,000 by dev-only rules; formal test remains unconsumed. This is not a completed MVP model: it did not train the full 226,218-record human M0. TD-16 is now an in-progress task group split into TD-16A through TD-16E for performance/capability-equivalence contract, full-human foundation, human-led distillation, repeated capability-equivalence selection, and one-shot formal test/MVP release; TD-17 and TD-18 remain pending. The frozen D1 composite contains 44,361 accepted train-only teacher targets. M0 v1, D0 v1, D1 v1, and the TD-16 A/B evidence remain immutable. The frozen tokenizer already has both Chinese tags and must not be rebuilt. Do not reinterpret the random deployment checkpoint, M1 memorization checkpoint, TD-16 A/B candidates, or full-M0 performance soaks as a trained final MVP model.
