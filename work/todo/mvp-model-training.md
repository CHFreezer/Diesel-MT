...
使用不可变的 `mvp-tokenizer-v0` 和从零初始化的 `mvp_e8_d2_v48k`，建立“近期 human parallel 来源 -> 确定性硬过滤 -> DeepSeek 长上下文批量找错 -> human-first 语料发布 -> 60M 翻译及格线 -> 重复能力等价 -> 独立评测 -> CTranslate2 float32/INT8”的第一个真实模型闭环。

本 todo 当前优先解决真实平行语料数量、年代、领域和语义忠实度，不预先冻结 teacher/human 比例或 synthetic 行数。只有 human-first 60M 基线完成并暴露具体弱路由后，才能另立一次有界数据补强；不包含无条件全量 DeepSeek/Hy-MT2 翻译、递归回译、在线 logits/hidden-state 蒸馏、64k tokenizer、`e12-d3`、约201.5M正式基线、生产质量门槛或移动端性能优化。64k/201.5M 是 MVP 通过后的计划，不是本 todo 已实施结果。

2026-07-21 重构：高吞吐训练器和旧数据长训诊断都已完成，但旧 MASSIVE M0 语义规模/领域不够，Hy-MT2 v3 又在 KFTT 实体与术语上失败。schema v4、旧数据、checkpoint、A/B 和 test 隔离证据保持不变；当前活动入口改为 TD-02A 近期来源清单，不再从 rejected teacher generation 继续修补。
...
- 官方 Hy-MT2 7B GGUF Q8_0 运行时、D0/D1 和 v3 rejected 生成继续冻结为历史诊断，不是新数据链依赖；student 不继承任何 teacher tokenizer、权重或架构。
- DeepSeek 在当前 TD-04 只对长上下文批次做稀疏问题 ID 标记，不自动重译或改写语料；现有 source-only 直译 A/B 只说明值得继续研究，不构成全量生成授权。
...
- 旧 D1 composite 的 44,361 条、schema v4 source bank 和 Hy-MT2 v3 accepted 只作诊断，不自动继承到新 human-first corpus。
- 新来源必须分别记录快照日期与内容年代；优先 2020 年以来及持续维护语料，但不得把新版本号冒充内容较新。
- 项目与权重按非商业研究用途设计可以扩大候选许可范围，但每个来源仍必须单独证明训练、衍生权重、署名/相同方式共享和数据再分发边界。

## 依赖关系

```text
TD-01 -> TD-02A 近期来源/许可清单 -> TD-02B pilot/实收率/预算
      -> TD-03 human parallel 预审构建
      -> TD-04 DeepSeek 长上下文批量找错
      -> TD-05 human-first corpus 发布
      -> TD-16C human-first 60M
      -> TD-16D 可选弱路由补强
      -> TD-16E 重复能力等价/选择
      -> TD-16F 一次性正式 test/MVP 发布
      -> TD-17 -> TD-18

TD-06～TD-08、TD-15、旧 TD-04 v1/v2/v3 = 不可变 teacher/蒸馏诊断支线
TD-09～TD-14、TD-16A = 已完成、可复用的训练/评测基础设施
```

历史执行已到 TD-16B：A/B诊断、可配置高吞吐训练器和完整旧M0长训均有证据，但TD-16B否决了旧M0作为通用MT foundation。TD-02/TD-03 schema v4 已完成来源实收、byte lock、source bank 与 human anchors；TD-04 v1 否决 KFTT 英文实体臆译，v2 因长目标截断停止，v3 则完整生成 195,404 条并通过数量门，但固定人工队列在 `jpn_Jpan→eng_Latn` 发现 KFTT 日文 source 的系统性专名/年号/术语臆译。v3 生成证据保持 rejected、禁止直接进入 TD-05。后续 512 条、20 路/60 strata 的 source-only A/B 已证明 DeepSeek V4 Flash 直译显著优于 Hy-MT2：119 条盲评为 56 胜/22 负/33 平/8 双差；当前只放行相同协议的 2,048 条复验，不能回写或自动放行 v3，也尚未授权 DeepSeek 全量生成。另一次同源 512 条 decode-only A/B 已排除“误用 greedy”假设：官方 sampling 的精确匹配校正 pass 为 463，低于 greedy 的 467；18 条真实差异盲评为 greedy 8胜、sampling 4胜、5平、1双差，且5条已知 KFTT blocker 只修复1条。因此不重跑 Hy-MT2 v3，旧TD-16继续 suspended。

2026-07-21 新增来源要求：重新打开 TD-02 的下一版来源合同，优先寻找内容年代可证明或持续更新、能够覆盖近年词汇的语料。准入不再只依赖确定性脚本清洗；候选通过硬门后按路线/来源/年代/领域分层合并成长上下文批次，DeepSeek 扫描整批后只返回疑似错误的样本 ID 和原因，不为正常句对逐条输出结论；再以 flag 全审、未标记样本抽检及已知错误 canary 校准长上下文漏检。DeepSeek 不得静默改写原文，也不能替代许可、污染和年代证据。schema v4 及已拒绝运行保持不可变。

## 当前重建门禁

- [ ] **[TD-02A](../task/mvp-model-training/td-02a-modern-corpus-inventory.md)**：建立近期平行语料来源、许可、快照/内容年代和路线清单。
- [ ] **[TD-02B](../task/mvp-model-training/td-02b-modern-corpus-pilot.md)**：小样本测真实可用率、唯一 groups/tokens、空间、API/人工成本，冻结正式预算。
- [ ] **[TD-03](../task/mvp-model-training/td-03-modern-corpus-build.md)**：下载并构建只经过确定性硬门的 human parallel `preaudit` corpus。
- [ ] **[TD-04](../task/mvp-model-training/td-04-deepseek-batch-audit.md)**：把大量句对合成长上下文批次，DeepSeek 只返回问题 ID；以 canary 和未标记抽查校准漏检。
- [ ] **[TD-05](../task/mvp-model-training/td-05-modern-corpus-acceptance.md)**：发布唯一 human-first 训练 manifest；实收不足则回到 TD-02，不以 synthetic 自动填洞。
- [x] schema v4、旧 TD-02～TD-05、D0/D1、Hy-MT2 v1/v2/v3 和旧 checkpoint 保持不可变，只作诊断证据。
...
### TD-02A / TD-02B 重建来源与预算

依赖：TD-01。详见 [TD-02 task group](../task/mvp-model-training/td-02-dataset-research-and-lock.md)。

- [ ] TD-02A 审计 OpenSubtitles v2024 近年子集、translatewiki、Mozilla、MDN、Wikimedia Content Translation、现有人类锚点及其他明确许可来源。
- [ ] TD-02A 为每个来源记录版本、snapshot/content date、语言/locale、许可/署名/非商业限制、规模、领域和风险，10 组关系均有 pilot 候选或明确缺口。
- [ ] TD-02A 同时记录两侧真人文本能否进入未来 64k tokenizer 候选池、文档边界和逐用途许可；`allow/hold/reject` 只表示候选资格，不表示已经准入或训练。
- [ ] TD-02B 做有界下载、硬门和人工抽检，按 groups/pairs/tokens 而非 directed rows 估算可用规模。
- [ ] TD-02B 以 5万～10万条 pilot 验证或修订首轮 90万～130万 independent pairs、可选扩展 150万～200万 pairs 的规划区间，并冻结逐关系、领域、新鲜度、单来源 ceiling 和磁盘/下载/API/人工预算。
- [ ] TD-02B 使用同一 pilot 复核 `mvp-tokenizer-v0` 的逐标签字符损失、`<unk>` offset、tokens/character、P95/P99 长度和零截断；通过则保持 49,152 身份，失败则阻塞并另立版本化 tokenizer 决策，不在本 task 原地扩词或重训。
- [ ] TD-02B 按稳定 text/document ID 统计未来 tokenizer 可复审的五标签唯一真人文本/字符量、年代和领域；排除 dev/test、holdout、synthetic、canary、quarantine 与正反路由重复。
- [ ] TD-02B 冻结 TD-04 的批次档位、人工抽查和质量门；暂定 critical canary recall ≥95%、flag precision ≥70%、未标记严重错误率 ≤1%，pilot 费用 10～30元、首轮累计 150～350元、扩展累计 300～600元，预计超过600元前重新确认。

完成条件：TD-02B 给出 `proceed` 才能进入 TD-03；不足则继续找来源，不用 synthetic 填满计划数字。

### TD-03 构建 human parallel 预审语料

依赖：TD-02B `proceed`。详见 [TD-03](../task/mvp-model-training/td-03-modern-corpus-build.md)。

- [ ] 复用共享数据模块实现各来源 adapter、断点下载、byte lock、规范化、时间/许可 metadata、group split 和确定性硬门。
- [ ] 输出不可训练的 `preaudit` 分片、硬门拒绝集、统计和 DeepSeek batch inventory；零截断，旧 schema v4 不混入。
- [ ] 两次独立构建的 corpus/split/manifest 字节一致；未知许可/版本拒绝，未知内容年代显式标记且不计入近期内容层。
- [ ] 构建与 directed records 分离的 tokenizer 候选账本，保留 train-side 真人文本引用、许可、日期、领域和哈希；不得让当前训练器消费，也不得称为 64k 训练集。

### TD-04 DeepSeek 长上下文批量语义审计

依赖：TD-02B、TD-03。详见 [TD-04](../task/mvp-model-training/td-04-deepseek-batch-audit.md)。

- [ ] 同路线/来源/领域的大量句对按 token 上限合批；DeepSeek 只稀疏返回问题 ID、严重度、类型和短理由，正常记录不回显。
- [ ] 先比较多个长上下文档位，以已知错误 canary、真实坏样本、未标记抽检和费用选择最大安全批次。
- [ ] 默认以 `deepseek-v4-flash` 比较约 64K/128K/256K input-token 档位；不因支持 1M context 就默认填满，执行前复核官方价格和模型身份。
- [ ] 全部 flag 人工复核，未标记样本分层抽检；响应截断、未知 ID、JSON 错误或缺失批次不得当作零问题。
- [ ] 只发布 audit overlay/quarantine，不自动重译或修改 TD-03 文本。

### TD-05 发布 human-first MVP corpus

依赖：TD-03、TD-04。详见 [TD-05](../task/mvp-model-training/td-05-modern-corpus-acceptance.md)。

- [ ] 发布真实平行 train/dev/test、许可/署名清单、audit 绑定和唯一 manifest；报告 groups/pairs/directed records/tokens、路线、年份、领域和简繁。
- [ ] 随 split 发布未来 tokenizer 候选账本，只保留 train-side 唯一真人文本；明确未来仍要补充广域单语来源并重新做许可、污染、配比、去重和规模饱和验收。
- [ ] dev/test 只含独立 human reference；正式 test 不被 DeepSeek、训练或调参消费。
- [ ] 实收不足则 `blocked` 并回到 TD-02；通过后才解锁 TD-16C human-first 训练。
...
完成记录：`scripts/mvp_student.py` 与统一入口 `scripts/mvp_cli.py validate-student` 已完成冻结 tokenizer 校验、方向感知编码/collator 和正式 student builder。TD-05 的 20 路 train composite CPU forward/backward loss 有限，双构造 state-dict SHA-256 均为 `66897f9c358802b9d39d66e61a8b39fad21236d11744b79df194c26db4da66a3`，离线保存/重载保持模型与 tokenizer 身份；机器记录为 `artifacts/model-training/reports/student/encoding-validation.json`，定向回归 `30 passed`。

### TD-10 实现训练循环、采样与运行记录

依赖：TD-09。

- [x] 实现 `scripts/mvp_training.py` 与统一入口 `scripts/mvp_cli.py train`，支持配置文件、dry-run、train/dev、固定 seed、设备/精度选择、梯度累积、gradient checkpointing、梯度裁剪和受控 dataloader worker。
...
- [x] 实现 `scripts/mvp_evaluation.py` 与统一入口 `scripts/mvp_cli.py evaluate`，离线加载数据、tokenizer 和 HF checkpoint；明确 dev/test 入口并默认拒绝在训练过程中读取 test。
...
恢复依赖：新的 TD-05 human-first corpus；TD-16C/TD-16D 必须在看到 TD-05 的实际 groups/tokens/路线/长度分布后重写执行配置。旧 TD-08/TD-15 只保留诊断证据。任务组见 [`td-16-m2-training.md`](../task/mvp-model-training/td-16-m2-training.md)。
...
- [ ] **TD-16C**：新 TD-05 完成后，从零使用唯一 human-first manifest 训练60M；训练步数、采样和长度 profile 按实际 tokens/路线冻结，按预注册 dev 能力线和 early-stop 选择 checkpoint。
- [ ] **TD-16D**：仅当 TD-16C 存在未过线弱路由时，先继续找 human 数据；仍不足且用户另行授权，才以全局约5%～10%、单弱路由不超过约20%的 synthetic 候选曝光，与等预算 human-only continuation 做一次 A/B。若首轮已过线则记录为无需执行。
- [ ] **TD-16E**：重复运行最终配方，以能力容差和 time-to-quality 验证统计等价并冻结唯一候选。
- [ ] **TD-16F**：只对唯一候选执行一次正式 test，发布最终 HF MVP、20路报告和不可重放 receipt。

产物：不可变 A/B/旧 M0 长训诊断、硬件可配置训练器、新 human-first corpus、60M 能力候选、可选弱路由补强、重复训练等价报告、唯一最终 HF MVP 和一次性正式 test 报告。

完成条件：TD-16A～TD-16F 全部完成；新 TD-05 human-first 60M 达到预注册总体/逐路由能力线，唯一候选通过重复能力等价和冻结 dev 选择，test 只消费一次。A/B 候选、旧 M0 checkpoint、性能 soak、训练完成或单次 loss 下降均不能单独完成 TD-16。
...
- [ ] 整理近期来源/许可/时间、human corpus、DeepSeek batch audit、历史 teacher/生成数据、student 模型、训练、评测、checkpoint、CT2 和离线包的配置/manifest/hash 关系，生成单一可追溯索引。
- [ ] 运行完整离线自动化测试和所有标记的慢速集成测试，记录命令、版本、测试数、耗时和结果；确认无运行产物或敏感/大体积数据被 Git 跟踪。
- [ ] 从干净临时目录验证 fixture 数据构建、DeepSeek batch 响应解析/稀疏 flag/canary、历史 teacher fixture、M1 短训练/恢复、评测和离线 CT2 冒烟可重复执行。
...
- [ ] 当前 TD-02A～TD-05、TD-16C～TD-16F、TD-17～TD-18 全部完成；旧 M0/D1/schema v4 只作为诊断，M1、M2、M3 阶段门槛依次通过。
- [ ] `mvp-tokenizer-v0` 冻结根保持 `eb79ae22f523f1d9c9fcf75b80f2b322e3c2882a8fddb7545b5933dd4053fa7f`，MVP/TD-16/TD-17 全链词表为 49,152 且 ID 顺序一致；不把该结论描述为后续 64k 正式基线已完成。
- [ ] MVP 数据覆盖 5 个标签桶、10 组无向模型关系和 20 个有向路由；简体、繁体分别有独立 dev/test，无 train/dev/test 泄漏。
- [ ] Hy-MT2 D0/D1/v3 与 DeepSeek 直译 A/B 保持可追溯且不混入新 human-first corpus；DeepSeek batch audit 的输入/flag/canary/抽检/费用可审计，正式 test 未发送给任何 teacher/API。
- [ ] M1 小样本过拟合、原子 checkpoint、故障拒绝和同环境恢复一致性通过。
- [ ] `mvp_e8_d2_v48k` 已完成旧共同 source A/B/长训诊断，并使用新 TD-05 human-first corpus 在 TD-16C 达到预注册能力线；最终配方通过重复能力等价后才选择唯一候选，test 只执行一次。
