...
2026-07-17 长训修正：高吞吐训练器已经合并，完整旧 M0 也已完成至 15k 的诊断长训；结果证明当前阻塞点是 TD-02 的来源适用性假设，而非 GPU/CPU 管线。旧 M0 的 226,218 条 directed records 只有 11,411 个 semantic/alignment groups，且 MASSIVE locale adaptation 允许实体本地化替换，不能继续充当通用翻译底模主体。当前活动任务正式回退到 TD-02，重新执行 TD-02～TD-05；TD-16 整体暂停，旧数据、checkpoint 和 test 隔离证据保持不变。
...
- D1 v1 保留 39,941 条 accepted。新增两路各固定 2,224 个候选、accepted 至少 2,000；只有引用 v1 与 addendum 的 20 路 composite 可进入已完成的 TD-15/原 TD-16 A/B 诊断。旧 D1/M0 不自动继承到修复后的 foundation；其用途必须由 TD-16C 新语料身份显式声明。
...
TD-16B -- 旧 M0 来源适用性失败 --> TD-02 v3 -> TD-03 v3 -> TD-04 v3 -> TD-05 v3
TD-05 v3 -> TD-16C -> TD-16D -> TD-16E -> TD-16F
TD-16F -> TD-17 -> TD-18
```

历史执行已到 TD-16B：A/B 诊断、可配置高吞吐训练器和完整旧 M0 长训均有证据，但 TD-16B 否决了旧 M0 作为通用 MT foundation。当前唯一活动任务是重新打开的 TD-02 v3；TD-03～TD-05 v3 pending，TD-16 suspended。只有新 M0 通过 TD-05 v3 后才可恢复训练。

## 当前回退门禁

- [ ] **TD-02 v3（in progress）**：重新调研并锁定通用 MT 平行来源，按 independent semantic groups 和忠实度而非路由展开数评估规模。
- [ ] **TD-03 v3**：为新来源适配构建管线，显式区分 literal parallel 与 localization parallel，发布全新 corpus identity。
- [ ] **TD-04 v3**：对多来源语义组重新切分、去重、泄漏与污染审计，冻结独立 human dev/test。
- [ ] **TD-05 v3**：以语义/实体/数字/否定忠实度和人工抽检重新验收 M0；未通过不得恢复 TD-16。
...
恢复依赖：TD-05 v3；teacher 辅助阶段还需重新验证 TD-08/TD-15 与新 source identity 的兼容性。任务组见 [`td-16-m2-training.md`](../task/mvp-model-training/td-16-m2-training.md)。
...
- [ ] **TD-16C**：TD-05 v3 完成后，从零使用新 M0 训练 human foundation，按新 dev 能力、实体/数字忠实度和 early-stop 冻结最佳 checkpoint。
- [ ] **TD-16D**：从新 foundation 比较继续 human-only 与低比例 human-led teacher 辅助；旧 D1 不自动准入新 source identity。
- [ ] **TD-16E**：重复运行最终配方，以能力容差和 time-to-quality 验证统计等价并冻结唯一候选。
- [ ] **TD-16F**：只对唯一候选执行一次正式 test，发布最终 HF MVP、20路报告和不可重放 receipt。

产物：不可变 A/B/旧 M0 长训诊断、硬件可配置训练器、修复后的 human corpus、human foundation、human-led 辅助候选、重复训练等价报告、唯一最终 HF MVP 和一次性正式 test 报告。

完成条件：TD-16A～TD-16F 全部完成；新 human corpus 通过语义多样性与忠实度验收，唯一候选通过重复能力等价和冻结 dev 选择，test 只消费一次。A/B 候选、旧 M0 长训 checkpoint、100-step 性能 soak 或单次 loss 下降均不能单独完成 TD-16。
...
- [ ] TD-01～TD-15、TD-16A～TD-16F、TD-17～TD-18 全部完成，旧 M0/D1 证据、新 human corpus、M1、M2、M3 阶段门槛依次通过。
...
- [ ] `mvp_e8_d2_v48k` 已完成旧共同 source A/B/长训诊断，并使用 TD-05 v3 验收的新 M0 在 TD-16C 训练 foundation；teacher target 仅在预注册的 human-led 辅助配方中竞争，最终配方通过重复能力等价后才选择唯一候选，test 只执行一次。
