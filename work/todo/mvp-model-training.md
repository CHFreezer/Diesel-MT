...
2026-07-17 长训修正：高吞吐训练器已经合并，完整旧 M0 也已完成至 15k 的诊断长训；结果证明当前阻塞点是语料身份而非 GPU/CPU 管线。旧 M0 的 226,218 条 directed records 只有 11,411 个 semantic/alignment groups，且 MASSIVE locale adaptation 允许实体本地化替换，不能继续充当通用翻译底模主体。当前任务回退到 TD-16C 语料准备，旧数据、checkpoint 和 test 隔离证据保持不变。
...
TD-16 A/B -> TD-16A -> TD-16B -> TD-16C -> TD-16D -> TD-16E -> TD-16F
TD-16F -> TD-17 -> TD-18
```

TD-01～TD-16B 已完成：A/B 诊断、可配置高吞吐训练器合并和完整旧 M0 长训均已有证据。TD-16B 的结果是否决旧 M0 作为通用 MT foundation，而不是接受其 checkpoint。当前唯一活动任务是 TD-16C 语料修复；只有 TD-16A～TD-16F 全部完成，M2/MVP 训练才关闭。M3 在 TD-17 完成后关闭。
...
### TD-16 训练并冻结基于合格语料能力的 MVP 模型

依赖：TD-05、TD-08、TD-12、TD-13、TD-14、TD-15。任务组拆分见 [`td-16-m2-training.md`](../task/mvp-model-training/td-16-m2-training.md)。
...
- [ ] **TD-16C**：重新调研、适配并冻结通用翻译 human corpus；分开报告 directed records、唯一文本和 semantic groups，MASSIVE 仅作为显式窄领域/路由辅助，重建独立 human dev 与忠实度门槛。
- [ ] **TD-16D**：从零使用修复后的 human corpus 训练 foundation，按新 dev 能力、实体/数字忠实度和 early-stop 冻结最佳 checkpoint。
- [ ] **TD-16E**：从新 foundation 比较继续 human-only 与低比例 human-led teacher 辅助，并重复运行最终配方，以能力容差和 time-to-quality 冻结唯一候选。
...
- [ ] `mvp_e8_d2_v48k` 已完成旧共同 source A/B/长训诊断，并使用 TD-16C 修复后的 human corpus 训练 foundation；teacher target 仅在预注册的 human-led 辅助配方中竞争，最终配方通过重复能力等价后才选择唯一候选，test 只执行一次。
