...
- D1 v1 保留 39,941 条 accepted。新增两路各固定 2,224 个候选、accepted 至少 2,000；只有引用 v1 与 addendum 的 20 路 composite 可进入 TD-15 和 TD-16 的 teacher-target/A/B 部分。TD-16B 的 human foundation 必须消费 TD-05 的完整 226,218 条 human M0。
...
TD-16 A/B -> TD-16A -> TD-16B -> TD-16C -> TD-16D -> TD-16E
TD-16E -> TD-17 -> TD-18
```

TD-01～TD-15 已完成；44,313 条共同 source 的 TD-16 等预算 A/B 诊断也已完成并选择 human-only step 1,000，但该运行未训练完整 226,218 条 human M0，不能称为完成 MVP 模型训练。TD-16 现为任务组，下一项是 TD-16A；只有 TD-16A～TD-16E 全部完成，M2/MVP 训练才关闭。M3 在 TD-17 完成后关闭。
...
- [ ] **TD-16A**：定版性能优先训练器与“重复训练能力等价、权重 hash 不必相同”的预注册验收合同。
- [ ] **TD-16B**：从零使用完整 226,218 条 human M0 训练 human foundation，按 dev 能力/early stop 冻结最佳 checkpoint。
- [ ] **TD-16C**：从 human foundation 执行 human 主导、teacher 低比例辅助的等墙钟/等 token 对照；同一 source 每次曝光只选择一个 target，不无条件双倍训练。
- [ ] **TD-16D**：重复运行最终配方，以总体和20路 dev 能力容差及 time-to-quality 验证统计等价，冻结唯一候选。
- [ ] **TD-16E**：只对唯一候选执行一次正式 test，发布最终 HF MVP、20路报告和不可重放 receipt。

产物：不可变 A/B 诊断、性能/能力等价合同、完整 human foundation、human-led 蒸馏候选、重复训练等价报告、唯一最终 HF MVP 和一次性正式 test 报告。

完成条件：TD-16A～TD-16E 全部完成；完整 human M0 已进入训练范围，唯一候选通过重复能力等价和冻结 dev 选择，test 只消费一次。A/B 候选、100-step 性能 soak 或单次 loss 下降均不能单独完成 TD-16。
...
- [ ] TD-01～TD-15、TD-16A～TD-16E、TD-17～TD-18 全部完成，M0、D0 smoke、D1 distilled mvp、M1、M2、M3 阶段门槛依次通过。
...
- [ ] `mvp_e8_d2_v48k` 已完成共同 source A/B 诊断，并使用完整 226,218 条 human M0 训练 foundation；teacher target 仅在预注册的 human-led 辅助配方中竞争，最终配方通过重复能力等价后才选择唯一候选，test 只执行一次。
...
- [ ] TD-01～TD-15、TD-16A～TD-16E、TD-17～TD-18 全部完成后，对本 todo 和完整 task 集合执行一次统一 review；不为单个 TD 提前创建 review。
