...
- `mvp`：覆盖全部 18 路由并达到预先冻结的逐路由数量/质量门槛，允许用于本 plan 的训练与 A/B 结论；
- `scale/production`：多来源、全量生成和正式质量验收，仍属于后续 plan。

在本 plan 中，TD-05 发布的 203,942 条 M0 human train 是 `mvp` 级人类语料；M0 里程碑还包含 TD-09 的编码契约，因此 TD-05 completed 不等于整个 M0 里程碑关闭。TD-08 已发布的 2,263 条 D0 accepted teacher targets 只属于 `smoke`，每路由 123～128 条，不得进入 TD-15/TD-16 的正式 A/B。D1 是已完成的 `mvp` 级蒸馏语料：从 M0 train 固定选择每路由 2,224 个候选（总计 40,032），最终接受 39,941 条，每路由 2,211～2,223 条，达到每路由至少 2,000 个 accepted、总 accepted 至少 36,000 的冻结门槛。D1 仍是有界 MVP，不等于全量或生产语料。
...
- D0 smoke 的 complete manifest 只证明真实生成链可复现，不赋予训练质量用途。D1 必须使用同一 teacher/prompt/decode/filter 身份，确定性扩展为 D0 source 集合的超集；每路由固定 2,224 个候选，接受率继续不得低于 0.90，且 accepted 不得少于 2,000，总 accepted 不得少于 36,000。当前 D1 已同时满足数量、质量、人工抽检、replay、provenance 和 test 隔离门槛；若其冻结身份或证据后续漂移，TD-08 必须重新退回 `in_progress`，TD-15/TD-16 不得消费 D0 代替 D1。
...
- 只对 train source 每路由生成 128 个候选，覆盖 18 个路由，raw/accepted/filtered 与完整 provenance 可追溯；
- D0 只验收生成、过滤、人工审查、checkpoint/resume、replay 和 manifest-last 发布，不作为 TD-15/TD-16 的最小可用训练 corpus。

### D1：Hy-MT2 7B 最小可用蒸馏语料

- D1 使用与 D0 相同且已冻结的 teacher、prompt、decode、filter 和 train-only/test-isolation 契约，候选 source 身份为 D0 的确定性超集；
- 18 路由各固定 2,224 个候选，总计 40,032；最终接受 39,941 条，每路由 2,211～2,223 条，接受率、脚本、重试、人工抽检和精确 replay 门槛全部通过；
- 已发布独立的 D1 raw/accepted/filtered、质量报告、审查证明和 complete manifest，未覆盖或把 D0 artifact 原地改名为 D1；
- 只有 D1 accepted targets 与对应 M0 human references 的交集可进入 TD-15，test 始终保持隔离。Git 跟踪的闭环证据为 `artifacts/model-training/td08-d1-distilled-data.json`。
...
- 在锁定且覆盖 9 组无向平行语料、18 个有向路由的 D1 MVP cohort 上分别完成 human-only baseline 与 Hy-MT2 7B distilled 候选训练；D0 smoke 不得替代 D1；
...
- Hy-MT2 7B teacher 能从锁定 artifact 完全离线运行；D0 smoke 证明生成链正确，D1 达到每路由至少 2,000 accepted 和总计至少 36,000 的 MVP 门槛；两者 raw/accepted/filtered 及 provenance 完整，test 从未送入 teacher。
...
- task：[MVP model training](../task/mvp-model-training/index.md)（TD-01 至 TD-08 completed；TD-09 至 TD-18 pending）。
