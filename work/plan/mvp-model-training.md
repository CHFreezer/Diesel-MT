...
2026-07-17 长训结论再次修正边界：可配置高吞吐训练器已合入主分支，完整旧 M0 也训练至 step 15,000 并 early-stop；训练稳定但 validation loss 在 step 4,000 后持续回升。进一步审计确认 226,218 条 directed records 只有 11,411 个 semantic/alignment groups，而且 MASSIVE 的 `utt` 是窄领域 locale adaptation，会合法替换地点、媒体、服务和人物。这种数据适合训练系统/路由验证，不足以承担通用机器翻译 foundation。失效前提最早位于 TD-02 的来源选择，因此当前正式重新打开 TD-02，并依次重做 TD-03～TD-05；TD-16 整体暂停，旧 corpus/checkpoint 均保持不可变诊断证据。
...
在本 plan 中，M0 v1 的 203,942 条 human train 是不可变的 18 路历史语料；TD-05 发布的 20 路 M0 共 226,218 条 directed train records。两者继续作为 route/system-validation 历史证据，但长训后不再满足通用 MT `mvp` 语料门槛。TD-08 的 2,263 条 D0 v1 accepted teacher targets 只属于 `smoke`。D1 v1 从 40,032 个候选接受 39,941 条；新增两路 addendum 从 4,448 个候选接受 4,420 条，最终 D1 composite 为 44,361 条、20 路 distilled 诊断数据。所有旧产物均不覆盖、不改名，也不自动继承到 TD-16C 的新语料身份。

## 目标

从零初始化并训练 `mvp_e8_d2_v48k` M2M100 语义 Encoder-Decoder 模型，依次完成小样本过拟合、Hy-MT2 7B D0/D1 与 human-only/distilled 诊断、旧 M0 长训否决、通用翻译 human corpus 修复、human foundation、human 主导的蒸馏辅助、重复训练能力等价、独立验证/一次性正式测试、Hugging Face 离线重载和 CTranslate2 CPU INT8 推理，形成后续全量蒸馏与更大模型训练可复用的稳定入口。
...
- 在重新执行的 TD-02～TD-05 冻结新 human corpus 后完成 foundation，并以预注册的低比例 teacher target 作为可选辅助监督；
...
### M2：A/B/旧 M0 诊断、回退数据链、human foundation 与 human-led 蒸馏
...
- 当前离开 TD-16，重新执行 TD-02 来源调研、TD-03 构建、TD-04 split/leakage 和 TD-05 M0 验收；按 independent semantic groups 和 source-target fidelity 验收，MASSIVE 只能作为显式窄领域/localization 辅助。
- TD-05 v3 完成后，TD-16C 使用新 M0 训练 human foundation，并以新冻结 human dev 和 early stop 选择 checkpoint。
- TD-16D 比较继续 human-only 与低比例 human-led teacher 辅助；TD-16E 通过独立复跑验证能力等价、冻结唯一候选。
...
- `mvp_e8_d2_v48k` 完成旧共同 source A/B/长训诊断、修复语料 human foundation、human-led teacher 辅助候选和重复能力等价验收；最终候选遵守预先冻结的新 dev/time-to-quality 与逐路由规则。
...
- 超出 TD-02～TD-05 v3 有界新语料预算的 production-scale 多来源扩充；
...
- task：[MVP model training](../task/mvp-model-training/index.md)（历史执行到 TD-16B；当前 TD-02 v3 in_progress、TD-03～TD-05 v3 pending、TD-16 suspended）。
