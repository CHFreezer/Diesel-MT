...
- 锁定 Hy-MT2 7B teacher 的模型/代码/许可证身份，验证可完全离线运行的本机或受控运行 profile；
...
- teacher 固定为腾讯官方 `tencent/Hy-MT2-7B` 系列的一个经验证运行 artifact。模型卡明确支持中文、繁体中文、英文、日文和韩文；项目映射分别为 `zho_Hans -> Chinese`、`zho_Hant -> Traditional Chinese`、`eng_Latn -> English`、`jpn_Jpan -> Japanese`、`kor_Hang -> Korean`，最终以逐路由脚本合规和参考译文校准结果为准。
- teacher artifact 必须锁定 Hugging Face revision、完整文件清单/SHA-256、许可证文件、remote code/chat template 和运行依赖。官方 Transformers 示例需要 `trust_remote_code=True`；因此远端代码必须先审查、固定哈希并缓存，正式生成阶段断网离线加载，不执行浮动 `main`。
...
本阶段允许 teacher 使用经验证的官方 FP8/GGUF 等运行 artifact 或 BF16 CPU/GPU offload，以适应 RTX 4060 Ti 16 GB；但必须记录量化/运行后端差异并在参考集上验收，不能用来源不明的社区量化产物替代官方 teacher 身份。
...
- teacher artifact、remote code、许可证、运行环境、语言名称映射、prompt 和 decode profile 全部锁定并可离线重载；
...
- teacher remote code/revision 未锁定、无法完全离线重载、16 GB 本机/受控 profile 无可接受运行路径或逐路由校准失败时，停止蒸馏数据生成，不退回浮动远端代码或来源不明量化模型。
...
- task：[MVP model training](../task/mvp-model-training/index.md)（TD-01 至 TD-18 已建立，均为 pending）。
