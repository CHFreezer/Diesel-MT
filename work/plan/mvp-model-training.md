...
- 在 TD-16C 修复并冻结的 human corpus 上完成 foundation，并以预注册的低比例 teacher target 作为可选辅助监督；
...
### M2：A/B/旧 M0 诊断、语料修复、human foundation 与 human-led 蒸馏
...
- TD-16C 重新准备 human corpus，按 independent semantic groups 和 source-target fidelity 验收；MASSIVE 只能作为显式窄领域/localization 辅助。
- TD-16D 使用修复语料训练 human foundation，并以新冻结 human dev 和 early stop 选择 checkpoint。
- TD-16E 比较继续 human-only 与低比例 human-led teacher 辅助，并通过独立复跑验证能力等价、冻结唯一候选。
...
- 超出 TD-16C 有界修复语料预算的 production-scale 多来源扩充；
...
- task：[MVP model training](../task/mvp-model-training/index.md)（TD-01～TD-16B completed；TD-16C 语料修复 in_progress；TD-16D～TD-18 pending）。
