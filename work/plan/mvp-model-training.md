...
2026-07-17 TD-16 边界修正：44,313 条共同 source 上的 1,000-step human-only/distilled A/B 已完成，但它只回答“teacher target 能否替代 human target”，没有训练完整 226,218 条 human M0，不能被称为已完成 MVP 模型训练。TD-16 拆为 TD-16A～TD-16E；只有完成性能/能力等价合同、完整 human foundation、human-led 蒸馏辅助、重复能力等价选择和一次性正式 test，才允许声明基于现有语料能力的 MVP 已训练完成。
...
在本 plan 中，M0 v1 的 203,942 条 human train 是不可变的 18 路历史语料；TD-05 已发布新的 20 路 M0，共 226,218 条 human train。TD-08 的 2,263 条 D0 v1 accepted teacher targets 只属于 `smoke`。D1 v1 从 40,032 个候选接受 39,941 条；新增两路 addendum 从 4,448 个候选接受 4,420 条，最终 D1 composite 为 44,361 条、20 路 distilled MVP。v1 产物均未被覆盖、改名或重新解释。

## 目标

从零初始化并训练 `mvp_e8_d2_v48k` M2M100 语义 Encoder-Decoder 模型，依次完成小样本过拟合、Hy-MT2 7B D0 冒烟与 D1 最小可用离线 sequence-level 蒸馏、human-only 与 distilled 等预算诊断、完整 226,218 条 human M0 foundation、human 主导的蒸馏辅助、重复训练能力等价、独立验证/一次性正式测试、Hugging Face 离线重载和 CTranslate2 CPU INT8 推理，形成后续全量蒸馏与更大模型训练可复用的稳定入口。
...
- 在完整 226,218 条 human M0 上完成 human foundation，并以预注册的低比例 teacher target 作为可选辅助监督；
...
- D0/D1 v1 的 complete manifest 只对既有 18 路身份成立。两条新增路线各固定 2,224 个候选，并已分别接受 2,213/2,207 条，通过质量、人工抽检、replay、provenance 和 test 隔离门槛。TD-15 与 TD-16 的 teacher-target/A/B 部分只允许消费冻结的 44,361 条 20 路 composite，不得用 D0 或单独的 D1 v1 代替；TD-16B 的 human foundation 必须消费 TD-05 的完整 human M0。
...
### M2：A/B 诊断、完整 human foundation 与 human-led 蒸馏
...
- TD-16A 在结果产生前冻结性能优先资源 profile、time-to-quality 和重复训练能力等价合同；训练热路径不追求逐 batch/权重 hash 一致。
- TD-16B 从零使用完整 226,218 条 human M0 训练 foundation，并以冻结 human dev 与 early stop 选择 checkpoint。
- TD-16C 只从 human foundation 比较继续 human-only 与预注册的低比例 human-led teacher 辅助；同一 source 每次曝光只选择一个 target，不无条件双倍加权。
- TD-16D 通过独立重复训练验证能力等价并冻结唯一候选；若 teacher 辅助不改善 time-to-quality 或触发任一路由红线，则选择 human-only。
- TD-16E 才能读取一次正式 test 并发布最终 MVP。无 NaN/Inf、无语言 token/词表越界，20路能力、峰值显存、吞吐和运行 manifest 必须齐全。
...
- `mvp_e8_d2_v48k` 完成共同 source A/B 诊断、完整 human M0 foundation、human-led teacher 辅助候选和重复能力等价验收；最终候选遵守预先冻结的 dev/time-to-quality 与逐路由规则。
...
- task：[MVP model training](../task/mvp-model-training/index.md)（TD-01～TD-15 completed；TD-16 A/B 诊断完成；TD-16A～TD-16E、TD-17～TD-18 pending）。
