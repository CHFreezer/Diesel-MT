...
- 本地保留选定 GGUF、llama.cpp 后端及原版 BF16 质量基线，统一放在工作目录下 Git-ignored 的 `artifacts/model-training/runtime/`。HDD 只承担模型文件到 RAM/VRAM 的顺序加载与低频只读访问；热 checkpoint、随机写缓存和频繁日志必须使用各自受控运行目录，不能写入模型快照目录。
