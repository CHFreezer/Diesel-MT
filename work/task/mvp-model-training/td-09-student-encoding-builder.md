...
- 当时由 `scripts/validate_mvp_student.py` 生成的验收现迁移为 `scripts/mvp_cli.py validate-student`：从 TD-05 的正式 20 路 train composite 各取一条固定记录，在 CPU 完成单 batch forward/backward；loss 为 `10.815247535705566`，全部梯度有限。随机 HF checkpoint 保存到 Git-ignored runtime 后完全离线重载，模型 state 与 tokenizer vocab 均未变化。
