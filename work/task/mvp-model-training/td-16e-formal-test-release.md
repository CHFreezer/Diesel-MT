# task TD-16E: 执行一次性正式 test 并发布 MVP
...
只对 TD-16D 冻结的唯一候选执行一次正式 test，发布第一个真正基于现有完整语料能力训练的 MVP HF 模型与质量报告。
...
- 报告20个标签路由、12个跨语言产品方向和2个简繁互转操作的 loss、BLEU/chrF、脚本合规、空输出、source copy和目标控制。
...
- test 只消费一次；TD-16A～TD-16E 全部完成后，TD-16 才可标记 `completed`，TD-17 方可开始。
