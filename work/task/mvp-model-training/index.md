...
```mermaid
flowchart LR
    TD01["TD-01 执行契约"] --> TD02["TD-02 质量优先来源调研（schema v4）"]
    TD02 --> TD03["TD-03 source/anchor 数据管线"]
    TD03 --> TD04["TD-04 teacher 与反向 pair"]
    TD04 --> TD05["TD-05 mixed corpus 验收"]
    ...
    TD05 --> TD16C["TD-16C mixed 60M 能力训练"]
    TD16C --> TD16D["TD-16D 可选弱路由 patch"]
...
| 9.0 | TD-16 | [训练并冻结基于合格语料能力的 MVP 模型（任务组）](td-16-m2-training.md) | TD-05 schema v4 completed | TD-05 schema v4 | 无 | suspended（待重写） |
...
| 9.3 | TD-16C | [训练 schema v4 mixed 60M 候选（文件待重写）](td-16c-repaired-human-foundation.md) | TD-05 schema v4 completed | TD-05 schema v4、TD-16A | 无 | pending |
| 9.4 | TD-16D | [执行或跳过一次弱路由 patch（文件待重写）](td-16d-human-led-distillation.md) | TD-16C completed | TD-16C、dev-only patch 门禁 | 无 | pending |
...
5. TD-03 只构建 source/anchors，TD-04 执行有界 teacher 生成，TD-05 只发布 mixed corpus；这三步都不启动 student 长训，正式 test 隔离保持不变。
6. TD-05 schema v4 完成后才恢复 TD-16C；正式 test 只允许未来 TD-16F 对 TD-16E 唯一候选读取一次。
